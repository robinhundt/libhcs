...
inline void mpz_zero(mpz_t op) { mpn_zero(op->_mp_d, op->_mp_alloc); }
...
int mpz_seed(mpz_t seed, int bits) {
  ...
  if (!fd) return HCS_EOPEN;

  const int bytes = (bits / 8) + 1;
  unsigned char random_bytes[bytes];
  if (fread(random_bytes, sizeof(random_bytes), 1, fd) != 1) return HCS_EREAD;
...
static void generator_gd_compute(mpz_t rop, mpz_t pi, mpz_t lambda, gmp_randstate_t rstate,
                                 mp_bitcnt_t bitcnt) {
  mpz_t t1;
  mpz_init(t1);
...
loop:
  mpz_add(rop, c, rho);
  if (mpz_even_p(rop)) mpz_add(rop, rop, nu);
...
void mpz_ripemd_3mpz_ul(mpz_t rop, mpz_t op1, mpz_t op2, mpz_t op3, unsigned long op4) {
  ...
  const size_t s1 = ((mpz_sizeinbase(op1, 2) + 7) / 8) * 8,
               s2 = ((mpz_sizeinbase(op2, 2) + 7) / 8) * 8,
               s3 = ((mpz_sizeinbase(op3, 2) + 7) / 8) * 8;

  const size_t toalloc = s1 > s2 ? s1 > s3 ? s1 : s3 : s2 > s3 ? s2 : s3;
