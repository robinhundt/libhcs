...
djcs_t_private_key *djcs_t_init_private_key(void) {
  ...
  vk->w = vk->l = vk->s = 0;
  mpz_inits(vk->p, vk->ph, vk->q, vk->qh, vk->v, vk->nsm, vk->m, vk->d, vk->delta, NULL);
...
void djcs_t_generate_key_pair(djcs_t_public_key *pk, djcs_t_private_key *vk, hcs_random *hr,
                              ...
                              const unsigned long w, const unsigned long l) {
...
void djcs_t_share_decrypt(djcs_t_private_key *vk, djcs_t_auth_server *au, mpz_t rop,
                          mpz_t cipher1) {
  mpz_t t1;
  mpz_init(t1);
...
void djcs_t_share_combine(djcs_t_private_key *vk, mpz_t rop, mpz_t *c) {
  ...
  for (unsigned long i = 0; i < vk->l; ++i) {
    if (mpz_cmp_ui(c[i], 0) == 0) continue; /* This share adds zero to the sum so skip. */

    /* Compute lambda_{0,i}^S. This is computed using the values of the
     * shares, not the indices? */
    mpz_set(t1, vk->delta);
    for (unsigned long j = 0; j < vk->l; ++j) {
      if ((j == i) || mpz_cmp_ui(c[j], 0) == 0) continue; /* i' in S\i and non-zero */
...
void djcs_t_clear_public_key(djcs_t_public_key *pk) { mpz_zero(pk->g); }

void djcs_t_clear_private_key(djcs_t_private_key *vk) {
  mpz_zeros(vk->p, vk->ph, vk->q, vk->qh, vk->v, vk->nsm, vk->m, vk->d, vk->delta, NULL);
  ...
  if (vk->n) {
    for (unsigned long i = 0; i <= vk->s; ++i) mpz_clear(vk->n[i]);
    free(vk->n);
  }
}
...
void djcs_t_free_public_key(djcs_t_public_key *pk) {
  ...
  if (pk->n) {
    for (unsigned long i = 0; i <= pk->s; ++i) mpz_clear(pk->n[i]);
    free(pk->n);
  }
...
void djcs_t_free_private_key(djcs_t_private_key *vk) {
  mpz_clears(vk->p, vk->ph, vk->q, vk->qh, vk->v, vk->nsm, vk->m, vk->d, vk->delta, NULL);
