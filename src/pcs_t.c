...
int pcs_t_generate_key_pair(pcs_t_public_key *pk, pcs_t_private_key *vk, hcs_random *hr,
                            const unsigned long bits, const unsigned long w,
  ...
  for (unsigned long i = 0; i < l; ++i) mpz_init(vk->vi[i]);
...
void pcs_t_r_encrypt(pcs_t_public_key *pk, hcs_random *hr, mpz_t rop, mpz_t r, mpz_t plain1) {
  mpz_t t1;
  mpz_init(t1);
...
void pcs_t_compute_ns_protocol(pcs_t_public_key *pk, hcs_random *hr, pcs_t_proof *pf, mpz_t cipher,
                               mpz_t cipher_r, unsigned long id) {
  mpz_t challenge, t1;
  mpz_init(challenge);
  mpz_init(t1);
...
int pcs_t_verify_ns_protocol(pcs_t_public_key *pk, pcs_t_proof *pf, unsigned long id) {
  ...
  if (mpz_cmp_ui(t1, 1) != 0) goto failure;
...
int pcs_t_verify_1of2_ns_protocol(pcs_t_public_key *pk, pcs_t_proof *pf, mpz_t cipher,
  ...
  if (mpz_cmp(esum, challenge) != 0) goto failure;
  ...
  if (mpz_cmp(t1, t2) != 0) goto failure;
...
pcs_t_polynomial *pcs_t_init_polynomial(pcs_t_private_key *vk, hcs_random *hr) {
  ...
  if ((px = malloc(sizeof(pcs_t_polynomial))) == NULL) goto failure;
  if ((px->coeff = malloc(sizeof(mpz_t) * vk->w)) == NULL) goto failure;
...
void pcs_t_free_polynomial(pcs_t_polynomial *px) {
  for (unsigned long i = 0; i < px->n; ++i) mpz_clear(px->coeff[i]);
  free(px->coeff);
  free(px);
}
...
void pcs_t_share_decrypt(pcs_t_public_key *pk, pcs_t_auth_server *au, mpz_t rop, mpz_t cipher1) {
  mpz_t t1;
  mpz_init(t1);
...
int pcs_t_share_combine(pcs_t_public_key *pk, mpz_t rop, hcs_shares *hs) {
  ...
  for (unsigned long i = 0; i < pk->l; ++i) {
    /* Skip zero shares */
    if (hs->flag[i] == 0) continue;

    /* Compute lagrange coefficients */
    mpz_set(t1, pk->delta);
    for (unsigned long j = 0; j < pk->l; ++j) {
      if ((j == i) || hs->flag[i] == 0) continue; /* i' in S\i and non-zero */
    ...
    if (mpz_sgn(t1) < 0 && !mpz_invert(t2, t2, pk->n2)) return 0;
  ...
  if (!mpz_invert(t1, t1, pk->n)) return 0;
...
void pcs_t_clear_private_key(pcs_t_private_key *vk) {
  mpz_zeros(vk->v, vk->nm, vk->n, vk->n2, vk->d, NULL);

  if (vk->vi) {
    for (unsigned long i = 0; i < vk->l; ++i) mpz_clear(vk->vi[i]);
    free(vk->vi);
  }
}
...
void pcs_t_free_private_key(pcs_t_private_key *vk) {
  mpz_clears(vk->v, vk->nm, vk->n, vk->n2, vk->d, NULL);
...
#define sz__(x) mpz_sizeinbase(pf->x, HCS_INTERNAL_BASE)
  ...
  const size_t buffer_len = HCS_MAX2(HCS_MAX3(sz__(e[0]), sz__(e[1]), sz__(a[0])),
