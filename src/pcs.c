...
pcs_private_key *pcs_init_private_key(void) {
  ...
  if (!vk) return NULL;
  mpz_inits(vk->p, vk->p2, vk->q, vk->q2, vk->hp, vk->hq, vk->mu, vk->lambda, vk->n, vk->n2, NULL);
  return vk;
}
...
void pcs_clear_public_key(pcs_public_key *pk) { mpz_zeros(pk->g, pk->n, pk->n2, NULL); }

void pcs_clear_private_key(pcs_private_key *vk) {
  mpz_zeros(vk->p, vk->p2, vk->q, vk->q2, vk->hp, vk->hq, vk->mu, vk->lambda, vk->n, vk->n2, NULL);
}

void pcs_free_private_key(pcs_private_key *vk) {
  pcs_clear_private_key(vk);
  mpz_clears(vk->p, vk->p2, vk->q, vk->q2, vk->hp, vk->hq, vk->mu, vk->lambda, vk->n, vk->n2, NULL);
  free(vk);
}
...
char *pcs_export_private_key(pcs_private_key *vk) {
  ...
  size_t buffer_size
      = mpz_sizeinbase((mpz_cmp(vk->p, vk->q) >= 0 ? vk->p : vk->q), HCS_INTERNAL_BASE) + 2;
