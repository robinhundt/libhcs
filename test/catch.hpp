...
namespace Catch {
  ...
  struct AutoReg {
    ...
    template <typename C> AutoReg(void (C::*method)(), char const* className,
...
#  define INTERNAL_CATCH_TESTCASE(...)                                                        \
    ...
    namespace {                                                                               \
      Catch::AutoReg INTERNAL_CATCH_UNIQUE_NAME(autoRegistrar)(                               \
          &INTERNAL_CATCH_UNIQUE_NAME(____C_A_T_C_H____T_E_S_T____), CATCH_INTERNAL_LINEINFO, \
...
// and your compiler supports SFINAE, try #defining CATCH_CONFIG_SFINAE
    ...
    template <typename T> struct IsStreamInsertable : IsStreamInsertableHelper<T>::type {};
...
#define INTERNAL_CHECK_THAT(arg, matcher, resultDisposition, macroName)                       \
  do {                                                                                        \
    Catch::ResultBuilder __catchResult(macroName, CATCH_INTERNAL_LINEINFO, #arg " " #matcher, \
                                       resultDisposition);                                    \
    try {                                                                                     \
        ...
        for (u_int m = 0; m < count; m++) {
          ...
          if (startsWith(methodName, "Catch_TestCase_")) {
            ...
            getMutableRegistryHub().registerTest(makeTestCase(new OcMethod(cls, selector),
                                                              className, name.c_str(), desc.c_str(),
                                                              SourceLineInfo()));
            noTestMethods++;
          }
        }
...
namespace Catch {
  ...
  public:
    ...
    Totals runTests() {
      ...
      getRegistryHub().getTestCaseRegistry().getFilteredTests(testSpec, *m_config, skippedTestCases,
                                                              true);
...
namespace Catch {
  ...
  void AutoReg::registerTestCase(ITestCase* testCase, char const* classOrQualifiedMethodName,
                                 NameAndDesc const& nameAndDesc, SourceLineInfo const& lineInfo) {
    getMutableRegistryHub().registerTest(
        makeTestCase(testCase, extractClassName(classOrQualifiedMethodName), nameAndDesc.name,
...
#  define INTERNAL_CATCH_REGISTER_LEGACY_REPORTER(name, reporterType)                         \
    namespace {                                                                               \
      Catch::LegacyReporterRegistrar<reporterType> catch_internal_RegistrarFor##reporterType( \
          name);                                                                              \
    }
...
#  define CATCH_CHECK_FALSE(expr)                                                                \
    INTERNAL_CATCH_TEST(                                                                         \
        expr, Catch::ResultDisposition::ContinueOnFailure | Catch::ResultDisposition::FalseTest, \
        "CATCH_CHECK_FALSE")
...
#  define CATCH_CHECK_THROWS_AS(expr, exceptionType)                                           \
    INTERNAL_CATCH_THROWS_AS(expr, exceptionType, Catch::ResultDisposition::ContinueOnFailure, \
...
#    define CATCH_FAIL(...)                                                                   \
      INTERNAL_CATCH_MSG(Catch::ResultWas::ExplicitFailure, Catch::ResultDisposition::Normal, \
