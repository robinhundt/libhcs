...
namespace hcs::pcs_t {
  ...
  public:
    explicit public_key(hcs::random &hr)
        : pk(pcs_t_init_public_key(), [](auto &ptr) { pcs_t_free_public_key(ptr); }), hr(hr) {}

    pcs_t_public_key *as_ptr() { return pk.get(); }

    hcs::random get_rand() { return hr; }
    ...
    void clear() { pcs_t_clear_public_key(pk.get()); }
  };
  ...
  public:
    explicit private_key(hcs::random &hr)
        : vk(pcs_t_init_private_key(), [](auto &ptr) { pcs_t_free_private_key(ptr); }), hr(hr) {}
    ...
    pcs_t_private_key *as_ptr() { return vk.get(); }

    hcs::random get_rand() const { return hr; }

    void clear() { pcs_t_clear_private_key(vk.get()); }
  };
  ...
  public:
    explicit polynomial(private_key &pk)
        : hr(pk.get_rand()), px(pcs_t_init_polynomial(pk.as_ptr(), hr.as_ptr()), [](auto &ptr) {
            pcs_t_free_polynomial(ptr);
          }) {}
    ...
    pcs_t_polynomial *as_ptr() { return px.get(); }
  ...
  public:
    auth_server(mpz_class &op, unsigned long id)
        : au(pcs_t_init_auth_server(), [](auto &ptr) { pcs_t_free_auth_server(ptr); }) {
    ...
    pcs_t_auth_server *as_ptr() { return au.get(); }

    mpz_class share_decrypt(public_key &pk, mpz_class &cipher1) {
      mpz_class rop;
      pcs_t_share_decrypt(pk.as_ptr(), au.get(), rop.get_mpz_t(), cipher1.get_mpz_t());
      return rop;
    }
  };

  inline void generate_key_pair(public_key &pk, private_key &vk, const unsigned long bits,
                                const unsigned long l, const unsigned long w) {
    pcs_t_generate_key_pair(pk.as_ptr(), vk.as_ptr(), vk.get_rand().as_ptr(), bits, w, l);
  }
